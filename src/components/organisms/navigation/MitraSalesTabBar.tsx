...
import { DollarSign, Home, User, Wallet } from 'lucide-react-native';
import React from 'react';
import { StyleSheet, TouchableOpacity, View } from 'react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import tw from 'twrnc';

// Maps route names to static icons to ensure consistency for Sales
...
export function MitraSalesTabBar({ state, descriptors, navigation }: BottomTabBarProps) {
    const insets = useSafeAreaInsets();
    const { user } = useAuth();

    // Explicitly whitelist ONLY the routes we want Sales to see
    ...
    // Filter down to allowed routes safely
    const visibleRoutes = state.routes.filter(route => {
        if (!ALLOWED_ROUTES.includes(route.name)) return false;

        // Dynamic checks
        if (route.name === 'marketing/canvasing/index') {
            // Ensure user has the canvasing feature and is marked as a sales user
            return user?.features?.includes(AppFeature.CANVASING) && user?.isSales;
        }
        ...
        <View style={[tw`absolute bottom-0 w-full px-6 pb-6 pt-2 bg-transparent`, { paddingBottom: insets.bottom > 10 ? insets.bottom : 24 }]}>
            <View style={[
                ...
                {visibleRoutes.map((route, visibleIndex) => {
                    ...
                    // Explicitly make the Canvasing tab the prominent center button for Sales
                    const isCenterItem = route.name === 'marketing/canvasing/index';
