...
import { Banknote, Calendar, CalendarDays, ClipboardCheck, Clock, Lock, Map, MessageCircle, PackageMinus, PackagePlus } from 'lucide-react-native';
            ...
            <View style={tw`flex-row flex-wrap justify-between`}>
                {menuItems.map((item, index) => {
                    ...
                    return (
                        ...
                        >
                            <View style={tw`h-10 w-10 rounded-lg ${enabled ? item.color : 'bg-gray-100'} items-center justify-center mb-2 relative`}>
                                <item.icon size={20} color={enabled ? item.iconColor : '#9ca3af'} />
                                {!enabled && (
                            ...
                            <Text style={tw`font-bold ${enabled ? 'text-gray-900' : 'text-gray-400'} text-xs text-center`} numberOfLines={1}>{item.title}</Text>
                            <Text style={tw`text-[10px] ${enabled ? 'text-gray-500' : 'text-gray-300'} text-center`} numberOfLines={1}>{item.subtitle}</Text>
                        </TouchableOpacity>
                    );
                })}
            </View>
