...
export default function AbsensiScreen() {
  ...
  const checkGeofence = (userLat: number, userLng: number) => {
    ...
    if (geofenceZones.length === 0) {
      console.log(
        "[Absensi] No zones found, setting isInside to true (fallback)",
      );
    ...
    for (const zone of geofenceZones) {
      ...
      console.log(
        `[Absensi] Distance to ${zone.siteName}: ${distance}m (radius: ${zone.radius}m)`,
      );
    ...
    console.log("[Absensi] Final geofence status:", {
      isInside,
      nearestDistance,
      nearestSiteName,
    });
  ...
  useEffect(() => {
    if (location && geofenceZones.length > 0) {
      console.log(
        "[Absensi] Re-checking geofence with zones:",
  ...
  const { data: statusData, refetch: refetchStatus } = useOfflineQuery<any>({
    ...
    fetcher: async () => {
      const res = await axios.get(
        `${Config.API_URL}/api/mobile/attendance/history?limit=1`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  useEffect(() => {
    if (statusData && statusData.success) {
      ...
      if (statusData.today && statusData.today.isHoliday) {
        setTodayHoliday({
          isHoliday: true,
          name: statusData.today.holidayName,
        });
      } else {
  ...
  const getLocation = async () => {
    try {
      ...
      if (status !== "granted") {
        Alert.alert(
          "Izin Ditolak",
          "Aplikasi membutuhkan izin lokasi untuk absensi.",
        );
        return;
      }
      ...
      Alert.alert(
        "Lokasi Error",
        "Pastikan GPS aktif. Di Emulator, set location di menu Extended Controls.",
      );
    }
  };
    ...
    if (cameraRef.current) {
      ...
      if (photo?.uri) {
        try {
          ...
          console.error(
            "[Absensi] Failed to flip photo, using original:",
            flipError,
          );
  ...
  const uploadPhotos = async (
    ...
    for (const uri of uris) {
      ...
      while (attempts <= retryCount && !success) {
        ...
        try {
          console.log(
            `[Absensi] Uploading photo attempt ${attempts}/${retryCount + 1}...`,
          );
          ...
          lastError = errorMsg;
          console.error(
            `[Absensi] Upload attempt ${attempts} failed:`,
            errorMsg,
          );
          ...
          if (attempts <= retryCount) {
            ...
            setLoadingMessage(
              `Mencoba upload ulang (${attempts}/${retryCount})...`,
            );
      ...
      if (!success) {
        console.error(
          "[Absensi] All upload attempts failed. Last error:",
          lastError,
        );
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    if (!photo || !location) {
      Alert.alert(
        "Data Belum Lengkap",
        "Pastikan foto dan lokasi sudah tersedia.",
      );
      return;
    }
    ...
    console.log(
      "[Absensi] User is INSIDE or no geofence, proceeding to submit",
    );
    // Proceed with submit
  ...
  const handleConfirmOutsideSubmit = async () => {
    ...
    if (!photo) {
      ...
      Alert.alert(
        "Foto Tidak Ada",
        "Foto tidak tersedia. Silakan ambil foto ulang.",
      );
      return;
    }
    ...
    if (!location) {
      ...
      Alert.alert(
        "Lokasi Tidak Ada",
        "Lokasi tidak tersedia. Silakan refresh halaman dan pastikan GPS aktif.",
      );
      return;
    }
    ...
    try {
      ...
      console.error(
        "[Absensi] Error saat submit dari geofence warning:",
        error,
      );
      setIsProcessing(false);
      Alert.alert(
        "Error",
        error?.message || "Gagal melakukan absensi. Silakan coba lagi.",
      );
    }
  };
  ...
  const submitAttendance = async () => {
    ...
    if (!photo || !location) {
      console.log(
        "[Absensi] submitAttendance - data tidak lengkap, photo:",
        !!photo,
        "location:",
        !!location,
      );
    ...
    if (isOnline) {
      ...
      try {
        ...
        if (!photoUrl)
          throw new Error("Gagal upload foto. Periksa koneksi internet Anda.");
          ...
          },
          {
            ...
            onSuccess: async (data: any) => {
              ...
              try {
                ...
                console.log(
                  "[Absensi] Tracking error (non-fatal):",
                  trackingError,
                );
              }
            ...
            onError: (e) => {
              ...
              Alert.alert(
                "Gagal",
                e.message || "Terjadi kesalahan saat mengirim data.",
              );
            },
          },
        );
        ...
        },
        {
          ...
          onSuccess: (data, isOffline) => {
            ...
            if (isOffline) {
              ...
              Alert.alert(
                "Offline",
                "Data disimpan dan akan dikirim saat online.",
              );
            }
          },
          onError: (e) => {
      ...
      return (
        <View style={tw`flex-1 justify-center items-center`}>
          ...
          <TouchableOpacity
            ...
            style={tw`bg-blue-600 p-2 rounded mt-2`}
          >
      ...
      <View style={tw`flex-1 bg-black`}>
        <CameraView style={tw`flex-1`} facing={facing} ref={cameraRef}>
          ...
          <View style={tw`absolute inset-0 items-center justify-center`}>
            <View
              style={[
                tw`w-56 h-72 border-2 border-white/60 rounded-full`,
                { borderStyle: "dashed" },
              ]}
            />
            <Text style={tw`text-white/80 text-xs mt-4`}>
          ...
          <View style={tw`absolute top-12 left-0 right-0 items-center`}>
            ...
            <Text style={tw`text-white/70 text-xs mt-1`}>
          ...
          <View style={tw`absolute bottom-0 left-0 right-0 p-6 pb-12`}>
            ...
            <View style={tw`bg-black/50 p-3 rounded-xl mb-4`}>
              <View style={tw`flex-row items-center`}>
                ...
                <Text
                  style={tw`text-white text-xs ml-2 flex-1`}
                  numberOfLines={1}
                >
                  {locationName}
                </Text>
            ...
            <View style={tw`flex-row justify-between items-center`}>
              ...
              >
                <View
                  style={tw`h-16 w-16 bg-white rounded-full border-2 border-gray-200`}
                />
              </TouchableOpacity>
  ...
  return (
    <SafeAreaView style={tw`flex-1 bg-gray-50`}>
      <ScrollView contentContainerStyle={tw`pb-20`}>
        {/* Header */}
        <View
          style={tw`${todayHoliday.isHoliday ? "bg-red-600" : "bg-blue-600"} px-6 pt-6 pb-12 rounded-b-[40px]`}
        >
          <View style={tw`items-center`}>
            <Text
              style={tw`${todayHoliday.isHoliday ? "text-red-100" : "text-blue-100"} font-medium text-sm mb-1`}
            >
              {format(currentTime, "EEEE, d MMMM yyyy", { locale: id })}
            </Text>
            ...
            {todayHoliday.isHoliday && (
              <View
                style={tw`bg-white/20 px-3 py-1 rounded-full mt-2 flex-row items-center`}
              >
        ...
        <View style={tw`px-4 -mt-8`}>
          ...
          >
            {/* Location */}
            <View
              style={tw`flex-row items-center bg-gray-50 p-3 rounded-xl mb-4`}
            >
            ...
            {photo ? (
              <View style={tw`mb-4`}>
                ...
                >
                  ...
                  {/* Watermark Overlay */}
                  <View
                    style={tw`absolute bottom-0 left-0 right-0 bg-black/60 p-3`}
                  >
                    <View style={tw`flex-row items-center mb-1`}>
                      <Clock size={12} color="#fff" />
                      <Text style={tw`text-white font-bold text-sm ml-2`}>
                        {capturedTime
                          ? format(capturedTime, "HH:mm:ss")
                          : "--:--:--"}
                      </Text>
                      <Text style={tw`text-white/80 text-xs ml-2`}>
                        {capturedTime
                          ? format(capturedTime, "EEEE, d MMMM yyyy", {
                              locale: id,
                            })
                          : ""}
                      </Text>
                ...
                <View style={tw`flex-row gap-2`}>
                  ...
                  >
                    <Text style={tw`font-bold text-white`}>
                      {loading || isMutating ? "Menyimpan..." : "Kirim Absensi"}
                    </Text>
                  </TouchableOpacity>
                </View>
              </View>
        ...
        <View style={tw`flex-1 bg-black/50 justify-center items-center px-6`}>
          <View style={tw`bg-white rounded-2xl p-6 w-full max-w-sm`}>
            ...
            {geofenceStatus && geofenceStatus.distance && (
              <View style={tw`bg-orange-50 p-3 rounded-xl mb-4`}>
                <Text style={tw`text-orange-700 text-center text-sm`}>
                  📍 Jarak:{" "}
                  {geofenceStatus.distance > 1000
                    ? `${(geofenceStatus.distance / 1000).toFixed(1)} km`
                    : `${geofenceStatus.distance} m`}{" "}
                  dari {geofenceStatus.siteName || "kantor"}
                </Text>
              </View>
            )}
            ...
            <View style={tw`flex-row gap-3`}>
              ...
              <TouchableOpacity
                onPress={() => {
                  ...
                  if (loading || isMutating) {
                    console.log(
                      "[Absensi] Would be disabled, but proceeding for debug",
                    );
                  }

                  handleConfirmOutsideSubmit();
                  console.log(
                    "[Absensi] handleConfirmOutsideSubmit called - END",
                  );
                }}
              ...
              >
                <Text style={tw`font-bold text-white`}>
                  {loading || isMutating ? "Menyimpan..." : "Lanjutkan"}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
