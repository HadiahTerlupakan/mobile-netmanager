...
const OvertimeItem = React.memo(({ item }: { item: Overtime }) => {
...
OvertimeItem.displayName = 'OvertimeItem';
...
export default function LemburScreen() {
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    await Promise.all([fetchData(), getLocation()]);
    setRefreshing(false);
  }, [fetchData, getLocation]);
  ...
  const handleCapture = useCallback(async () => {
    if (cameraRef.current) {
      const result = await cameraRef.current.takePictureAsync({ quality: 0.7, skipProcessing: false });
  ...
  const ListHeader = useMemo(() => (
    <View>
      <View style={tw`bg-indigo-600 px-6 pt-6 pb-12 rounded-b-[40px]`}>
        ...
        <DigitalClock />
        <Text style={tw`text-indigo-200 text-xs mt-1 text-center`}>Lembur</Text>
      </View>
            ...
            <>
              ...
              {todayRequest?.status === "APPROVED" && (
                <View style={tw`mb-4`}>
                  ...
                  {!canStartOvertime && <Text style={tw`text-amber-600 text-xs text-center mt-2`}>⚠️ Checkout absen dulu sebelum mulai</Text>}
                </View>
              )}
    ...
    if (!permission?.granted) {
      return (
        <View style={tw`flex-1 justify-center items-center`}>
          ...
          <TouchableOpacity onPress={requestPermission} style={tw`bg-indigo-600 p-2 rounded mt-2`}><Text style={tw`text-white`}>Izinkan</Text></TouchableOpacity>
        </View>
      );
    }
  ...
  return (
    <SafeAreaView style={tw`flex-1 bg-gray-50`} edges={["top"]}>
      <FlashList
        ...
        ListEmptyComponent={<Text style={tw`text-gray-400 text-center py-4`}>Belum ada data.</Text>}
      />
