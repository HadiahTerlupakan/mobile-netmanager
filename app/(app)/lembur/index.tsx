...
export default function LemburScreen() {
  ...
  const [activeAction, setActiveAction] = useState<"start" | "stop" | null>(
    null,
  );
  ...
  useEffect(() => {
    if (overtimeData) {
      ...
      const today = (overtimeData.history || []).find(
        (item: Overtime) =>
          item.createdAt.startsWith(todayStr) || item.status === "IN_PROGRESS",
      );
      ...
      return (
        <View style={tw`flex-1 justify-center items-center`}>
          ...
          <TouchableOpacity
            ...
            style={tw`bg-indigo-600 p-2 rounded mt-2`}
          >
              ...
              <View style={tw`flex-row justify-between mb-4`}>
                <View style={tw`items-center flex-1 border-r border-gray-100`}>
                  ...
                  <Text style={tw`text-lg font-bold text-gray-800`}>
                    {todayRequest.startTime
                      ? format(new Date(todayRequest.startTime), "HH:mm")
                      : "--:--"}
                  </Text>
                </View>
                ...
                <View style={tw`flex-row gap-2`}>
                  ...
                  >
                    <Text style={tw`font-bold text-white`}>
                      {showLoading ? "Memproses..." : "Konfirmasi"}
                    </Text>
                  </TouchableOpacity>
                </View>
              </View>
              ...
              <>
                ...
                {(!todayRequest ||
                  ...
                  >
                    ...
                    <Text style={tw`text-indigo-600 font-bold mt-2`}>
                      Ajukan Lembur
                    </Text>
                    <Text style={tw`text-indigo-400 text-xs`}>
                ...
                {todayRequest?.status === "PENDING" && (
                  <View
                    style={tw`bg-yellow-50 border border-yellow-200 rounded-2xl p-4 items-center mb-4`}
                  >
                    <Timer size={32} color="#ca8a04" />
                    <Text style={tw`text-yellow-700 font-bold text-lg mt-2`}>
                      Menunggu Approval
                    </Text>
                ...
                {todayRequest?.status === "APPROVED" && (
                  <View style={tw`mb-4`}>
                    <View
                      style={tw`bg-green-50 border border-green-200 rounded-xl p-3 flex-row items-center mb-3`}
                    >
                      <CheckCircle size={20} color="#16a34a" />
                      <View style={tw`ml-3`}>
                        <Text style={tw`font-bold text-sm text-green-700`}>
                          Disetujui
                        </Text>
                        <Text style={tw`text-xs text-green-600`}>
                          Silakan mulai saat jam lembur tiba.
                        </Text>
                      </View>
                    </View>
                    <TouchableOpacity
                      ...
                      style={[
                        ...
                        canStartOvertime
                          ? tw`bg-indigo-50 border-2 border-dashed border-indigo-200`
                          : tw`bg-gray-100 border-2 border-dashed border-gray-200`,
                      ]}
                    >
                      <Camera
                        size={32}
                        color={canStartOvertime ? "#4f46e5" : "#9ca3af"}
                      />
                      <Text
                        style={
                          canStartOvertime
                            ? tw`text-indigo-600 font-bold mt-2`
                            : tw`text-gray-400 font-bold mt-2`
                        }
                      >
                        Mulai Lembur
                      </Text>
          ...
          <View style={tw`mt-4`}>
            ...
            {history.length === 0 ? (
              ...
              history.slice(0, 5).map((item) => (
                <View
                  key={item.id}
                  style={tw`bg-white p-4 rounded-xl mb-2 flex-row justify-between items-center border border-gray-100`}
                >
                  ...
                  <View
                    style={[tw`px-2 py-1 rounded`, getStatusColor(item.status)]}
                  >
                    <Text
                      style={[
      ...
      <Modal visible={showRequestModal} transparent animationType="fade">
        <View style={tw`flex-1 bg-black/50 justify-center items-center p-4`}>
          <View style={tw`bg-white w-full max-w-sm rounded-2xl p-6`}>
            ...
            <TextInput
              style={[
                tw`w-full p-3 rounded-lg border border-gray-200 bg-gray-50 mb-4`,
            ...
            <View style={tw`flex-row gap-3`}>
              ...
              <TouchableOpacity
                style={[
                  ...
                  reason.trim() ? tw`bg-indigo-600` : tw`bg-gray-300`,
                ]}
