  ...
  return (
    <View style={tw`flex-1 bg-slate-50`}>
      ...
      >
        ...
        <View
          style={[
            tw`bg-slate-900 pb-16 px-4 rounded-b-[40px]`,
        ...
        >
          <View style={tw`flex-row items-center justify-between`}>
            ...
            <Text style={tw`text-lg font-black text-white uppercase tracking-widest`}>
              Canvasing Baru
            </Text>
            <View style={tw`w-10`} />
          </View>
        ...
        <ScrollView
          style={tw`flex-1 -mt-10 z-10`}
          contentContainerStyle={tw`pb-32 px-4`}
          showsVerticalScrollIndicator={false}
        >
          {/* Floating Hero Info */}
          <View style={tw`bg-white rounded-[32px] p-6 mb-6 shadow-sm border border-slate-100`}>
            <View style={tw`flex-row items-center mb-2`}>
              <View style={tw`w-10 h-10 bg-emerald-50 rounded-2xl items-center justify-center mr-3 border border-emerald-100`}>
                <Package size={20} color="#059669" />
              </View>
              <View>
                <Text style={tw`text-slate-800 text-xl font-black tracking-tight`}>
                  Input Data
                </Text>
            ...
            <Text style={tw`text-slate-500 text-xs font-medium leading-relaxed mt-2`}>
              Pastikan semua informasi KTP dan alamat pelanggan valid. Data ini akan masuk ke daftar verifikasi canvasing.
            </Text>
          </View>
          ...
          <TouchableOpacity
            ...
            style={tw`bg-emerald-500 p-5 rounded-[24px] shadow-sm shadow-emerald-200/50 flex-row items-center justify-center ${isLoading || isMutating ? "opacity-50" : ""}`}
          >
            {isLoading || isMutating ? (
              ...
              <>
                ...
                <Text style={tw`text-white font-black text-base ml-3 uppercase tracking-wider`}>
                  Submit Request
                </Text>
              </>
            )}
          ...
          {/* Hint */}
          <View style={tw`flex-row items-center justify-center mt-6 mb-8`}>
            <Info size={14} color="#94a3b8" />
            <Text
              style={tw`text-slate-400 text-[10px] font-bold uppercase ml-2 tracking-widest`}
            >
...
function FormSectionHeader({ title, icon }: FormSectionHeaderProps) {
  return (
    <View style={tw`flex-row items-center mb-4 ml-1`}>
      <View
        style={tw`w-8 h-8 bg-slate-100 items-center justify-center rounded-xl mr-3 border border-slate-200/50`}
      >
        {icon}
      </View>
      <Text style={tw`text-base font-black text-slate-800 tracking-tight`}>{title}</Text>
    </View>
  );
}
...
function InputField({
  ...
  return (
    <View style={tw`mb-5`}>
      <Text
        style={tw`text-[10px] text-slate-400 font-black uppercase tracking-widest mb-2 ml-1`}
      >
        {label} {required && <Text style={tw`text-rose-500`}>*</Text>}
      </Text>
      <TextInput
        style={tw`bg-slate-50 border border-slate-100/80 rounded-2xl p-4 text-sm text-slate-900 font-bold`}
...
function PhotoPickerField({
  ...
  return (
    <View>
      <Text
        style={tw`text-[10px] text-slate-400 font-black uppercase tracking-widest mb-3 ml-1`}
      >
        {title} {required && <Text style={tw`text-rose-500`}>*</Text>}
      </Text>
      {value ? (
        <View style={tw`bg-slate-50 p-2 rounded-3xl border border-slate-100`}>
          <View
            style={tw`relative rounded-2xl overflow-hidden aspect-video bg-slate-200`}
          >
            ...
            {onScan && (
              <View style={tw`absolute bottom-3 right-3 left-3`}>
                <TouchableOpacity
                  onPress={onScan}
                  disabled={isScanning}
                  style={tw`bg-emerald-600/90 p-3 rounded-xl flex-row items-center justify-center border border-white/20`}
                >
                  {isScanning ? (
        ...
        <View style={tw`flex-row gap-3`}>
          <TouchableOpacity
            ...
            activeOpacity={0.7}
            style={tw`flex-1 h-20 items-center justify-center bg-emerald-50/50 border border-dashed border-emerald-200 rounded-2xl gap-2`}
          >
            <View style={tw`bg-emerald-100 p-2 rounded-lg`}>
              <Camera size={18} color="#059669" />
            </View>
            <Text style={tw`text-emerald-700 text-[10px] font-black uppercase tracking-wider`}>
              Kamera
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
